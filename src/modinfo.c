...
sr_modinfo_module_srmon_module(sr_main_shm_t *main_shm, char *ext_shm_addr, sr_mod_t *shm_mod, struct lyd_node *sr_state)
{
    ...
    /* module */
    SR_CHECK_LY_RET(lyd_new_inner(sr_state, NULL, "module", 0, &sr_mod), ly_ctx, err_info);

    /* name */
    SR_CHECK_LY_RET(lyd_new_term(sr_mod, NULL, "name", ext_shm_addr + shm_mod->name, 0, NULL), ly_ctx, err_info);
    ...
    for (ds = 0; ds < SR_DS_COUNT; ++ds) {
        ...
        for (i = 0; i < shm_mod->change_sub[ds].sub_count; ++i) {
            /* change-sub */
            SR_CHECK_LY_RET(lyd_new_inner(sr_subs, NULL, "change-sub", 0, &sr_sub), ly_ctx, err_info);
            ...
            SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "pid", buf, 0, NULL), ly_ctx, err_info);
        }
    }
    ...
    for (i = 0; i < shm_mod->oper_sub_count; ++i) {
        /* operational-sub */
        SR_CHECK_LY_RET(lyd_new_inner(sr_subs, NULL, "operational-sub", 0, &sr_sub), ly_ctx, err_info);

        /* xpath */
        SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "xpath", ext_shm_addr + oper_sub[i].xpath, 0, NULL), ly_ctx, err_info);
...
sr_modinfo_module_srmon_rpc(sr_main_shm_t *main_shm, char *ext_shm_addr, sr_rpc_t *shm_rpc, struct lyd_node *sr_state)
{
    ...
    /* rpc */
    SR_CHECK_LY_RET(lyd_new_inner(sr_state, NULL, "rpc", 0, &sr_rpc), ly_ctx, err_info);

    /* path */
    SR_CHECK_LY_RET(lyd_new_term(sr_rpc, NULL, "path", ext_shm_addr + shm_rpc->op_path, 0, NULL), ly_ctx, err_info);

    rpc_sub = (sr_rpc_sub_t *)(ext_shm_addr + shm_rpc->subs);
    for (i = 0; i < shm_rpc->sub_count; ++i) {
        /* rpc-sub */
        SR_CHECK_LY_RET(lyd_new_inner(sr_rpc, NULL, "rpc-sub", 0, &sr_sub), ly_ctx, err_info);
        ...
        struct lyd_node *sr_state)
{
    ...
    /* connection */
    SR_CHECK_LY_RET(lyd_new_inner(sr_state, NULL, "connection", 0, &sr_conn), ly_ctx, err_info);
    ...
    for (i = 0; i < main_shm->mod_count; ++i) {
        for (ds = 0; ds < SR_DS_COUNT; ++ds) {
            ...
            /* module-lock */
            SR_CHECK_LY_RET(lyd_new_inner(sr_conn, NULL, "module-lock", 0, &sr_modlock), ly_ctx, err_info);

            /* name */
            SR_CHECK_LY_RET(lyd_new_term(sr_modlock, NULL, "name", ext_shm_addr + shm_mod[i].name, 0, NULL), ly_ctx, err_info);

            /* datastore */
            SR_CHECK_LY_RET(lyd_new_term(sr_modlock, NULL, "datastore", sr_ds2ident(ds), 0, NULL), ly_ctx, err_info);
...
sr_modinfo_generate_config_change_notif(struct sr_mod_info_s *mod_info, sr_session_ctx_t *session)
{
    ...
    while (!(err_info = sr_diff_set_getnext(set, &idx, &elem, &op)) && elem) {
        /* edit (list instance) */
        if (lyd_new_inner(notif, NULL, "edit", 0, &root)) {
            sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
            goto cleanup;
        }
